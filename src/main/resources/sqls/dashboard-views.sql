...
SELECT     { fn CONCAT(CONVERT(VARCHAR, TRG.YEAR), CONVERT(VARCHAR, TRG.PERIOD_NO)) } AS YEARPERIODNO,PS.AGE,PS.REPORTCODE, TRG.YEAR, TRG.PERIOD_NO, COUNT(SOH.SEQNO) AS QUOTEQTY, 
                      ISNULL(SUM(SOH.SUBTOTAL), 0) AS QUOTETOTAL, ISNULL(SUM(SOH.X_GP), 0) AS QUOTEPROFIT, ISNULL(SUM(TRG.MTD_QUOTE_VAL), 0) AS QUOTEVALUETARGET, 
                      ISNULL(SUM(TRG.MTD_QUOTE_QTY), 0) AS QUOTEQTYTARGET, ISNULL(SUM(TRG.MTD_QUOTE_PROFIT), 0) AS QUOTEPROFITTARGET, ISNULL(SUM(TRG.NO_OF_WORKING_DAYS), 0) 
                      AS NOOFWORKINGDAYS
FROM         dbo.X_Targets AS TRG LEFT OUTER JOIN
                      ...
                      MONTH(DATEADD(m, - 6, SOH.ORDERDATE)) = TRG.PERIOD_NO) AND (SOH.X_QUOTE_CREATED = 'Y' OR SOH.STATUS = 3)
                       LEFT OUTER JOIN
                      dbo.PERIOD_STATUS AS PS ON PS.PERIOD_SEQNO=TRG.PERIOD_NO AND YEAR(PS.STARTDATE)=TRG.YEAR AND YEAR(PS.STARTDATE)=YEAR(SOH.ORDERDATE) AND MONTH(PS.STARTDATE)=MONTH(SOH.ORDERDATE) AND PS.LEDGER='C' 
GROUP BY { fn CONCAT(CONVERT(VARCHAR, TRG.YEAR), CONVERT(VARCHAR, TRG.PERIOD_NO)) },PS.AGE,PS.REPORTCODE, TRG.YEAR, TRG.PERIOD_NO


-- X_VW_OS_SALEORDERS_BYPERIOD  
  SELECT     { fn CONCAT(CONVERT(VARCHAR, TRG.YEAR), CONVERT(VARCHAR, TRG.PERIOD_NO)) } AS YEARPERIODNO,PS.AGE,PS.REPORTCODE, TRG.YEAR, TRG.PERIOD_NO, COUNT(SOH.SEQNO) AS ORDERQTY, 
                      ISNULL(SUM(SOH.SUBTOTAL), 0) AS ORDERTOTAL, ISNULL(SUM(SOH.X_GP), 0) AS ORDERPROFIT, ISNULL(SUM(TRG.MTD_ORDER_VAL), 0) AS ORDERVALUETARGET, 
                      ISNULL(SUM(TRG.MTD_ORDER_QTY), 0) AS ORDERQTYTARGET, ISNULL(SUM(TRG.MTD_ORDER_PROFIT), 0) AS ORDERPROFITTARGET, ISNULL(SUM(TRG.NO_OF_WORKING_DAYS), 0) 
                      AS NOOFWORKINGDAYS
FROM         dbo.X_Targets AS TRG LEFT OUTER JOIN
                      dbo.SALESORD_HDR AS SOH ON YEAR(SOH.ORDERDATE) = TRG.YEAR AND (MONTH(DATEADD(m, 6, SOH.ORDERDATE)) = TRG.PERIOD_NO OR
...
SELECT     { fn CONCAT(CONVERT(VARCHAR, TRG.YEAR), CONVERT(VARCHAR, TRG.PERIOD_NO)) } AS YEARPERIODNO,PS.AGE,PS.REPORTCODE, TRG.YEAR, TRG.PERIOD_NO, COUNT(SOHTRN.SEQNO) AS INVOICEQTY,
                       ISNULL(SUM(SOHTRN.SUBTOTAL), 0) AS INVOICETOTAL, ISNULL(SUM(SOHTRN.X_GP), 0) AS INVOICEPROFIT, ISNULL(SUM(TRG.MTD_INV_VAL), 0) AS INVOICEVALUETARGET, 
                      ISNULL(SUM(TRG.MTD_INV_QTY), 0) AS INVOICEQTYTARGET, ISNULL(SUM(TRG.MTD_INV_PROFIT), 0) AS INVOICEPROFITTARGET, ISNULL(SUM(TRG.NO_OF_WORKING_DAYS), 0) 
                      AS NOOFWORKINGDAYS
FROM         dbo.X_Targets AS TRG LEFT OUTER JOIN
                          (SELECT     TRN.TRANSDATE, TRN.SEQNO, TRN.SUBTOTAL, SOH.X_GP
                            FROM          dbo.SALESORD_HDR AS SOH INNER JOIN
                                                   dbo.DR_TRANS AS TRN ON SOH.SEQNO = TRN.SO_SEQNO
                            WHERE      (TRN.TRANSTYPE = 1)) AS SOHTRN ON YEAR(SOHTRN.TRANSDATE) = TRG.YEAR AND (MONTH(DATEADD(m, 6, SOHTRN.TRANSDATE)) = TRG.PERIOD_NO OR
                      MONTH(DATEADD(m, - 6, SOHTRN.TRANSDATE)) = TRG.PERIOD_NO)
                      LEFT OUTER JOIN 
                      dbo.PERIOD_STATUS AS PS ON PS.PERIOD_SEQNO=TRG.PERIOD_NO AND YEAR(PS.STARTDATE)=TRG.YEAR AND YEAR(PS.STARTDATE)=YEAR(SOHTRN.TRANSDATE) AND MONTH(PS.STARTDATE)=MONTH(SOHTRN.TRANSDATE) AND PS.LEDGER='C' 
GROUP BY { fn CONCAT(CONVERT(VARCHAR, TRG.YEAR), CONVERT(VARCHAR, TRG.PERIOD_NO)) },PS.AGE,PS.REPORTCODE,  TRG.YEAR, TRG.PERIOD_NO
